...
Consulta: externa e interna (Subconsulta)
JOIN- unión de consultas Individuales 
IN- Rango implícito
CASE- evaluación de condición
WHERE => para filtrar filas antes de la agrupación y no puede contener funciones agregadas.
FUNCIONES AGREGADAS como Count, Sum...*GROUP BY 
	=AGRUPA las filas en grupos basados en los valores de una o más columnas
HAVING Filtra los resultados de las funciones agregadas.
	Basados en una condición, generalmente de función agregada.
Múltiple
Encapsular

Para la Evaluación Final del Módulo 2 he utilizado la Base de Datos Sakila.
Esta base de datos simula una tienda de alquiler de películas. Contiene
tablas como film (películas), actor (actores), customer (clientes), rental (alquileres),
category (categorías), etc. que he usado para realizar las consultas y análisis de datos requeridos para evaluación.
